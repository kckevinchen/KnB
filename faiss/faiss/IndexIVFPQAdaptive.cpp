...
#include <faiss/IndexIVFPQAdaptive.h>
...
/*****************************************
 * IndexIVFPQAdaptive implementation
 ******************************************/

IndexIVFPQAdaptive::IndexIVFPQAdaptive(
        Index* quantizer,
        size_t d,
        size_t nlist,
        size_t M,
...
void IndexIVFPQAdaptive::preset_level(int n, const int *assign_level, int max_level, const int *budget_per_level){
    pq.preset_level(n, assign_level,max_level, budget_per_level);
}

void IndexIVFPQAdaptive::train_residual(idx_t n, const float* x) {
    train_residual_o(n, x, nullptr);
}

void IndexIVFPQAdaptive::train_residual_o(idx_t n, const float* x, float* residuals_2) {
    const float* x_in = x;
...
void IndexIVFPQAdaptive::encode(idx_t key, const float* x, uint8_t* code) const {
    if (by_residual) {
...
void IndexIVFPQAdaptive::encode_multiple(
        size_t n,
        idx_t* keys,
        const float* x,
        uint8_t* xcodes,
...
void IndexIVFPQAdaptive::decode_multiple(
        size_t n,
        const idx_t* keys,
...
void IndexIVFPQAdaptive::add_core(
        idx_t n,
        const float* x,
        const idx_t* xids,
...
void IndexIVFPQAdaptive::encode_vectors(
        idx_t n,
        const float* x,
...
void IndexIVFPQAdaptive::sa_decode(idx_t n, const uint8_t* codes, float* x) const {
    size_t coarse_size = coarse_code_size();
...
void IndexIVFPQAdaptive::add_core_o(
        idx_t n,
        const float* x,
        const idx_t* xids,
        float* residuals_2,
    ...
    if (n > bs) {
        for (idx_t i0 = 0; i0 < n; i0 += bs) {
            ...
            if (verbose) {
                printf("IndexIVFPQAdaptive::add_core_o: adding %" PRId64 ":%" PRId64
                       " / %" PRId64 "\n",
                       i0,
                       i1,
                       n);
...
void IndexIVFPQAdaptive::reconstruct_from_offset(
        int64_t list_no,
        int64_t offset,
        float* recons) const {
 ...
 * quantizer is a MultiIndexQuantizerAdatpive then the table can be stored
 * more compactly.
 *
...
void initialize_IVFPQ_precomputed_table(
    ...
    if (use_precomputed_table == 0) { // then choose the type of table
        if (quantizer->metric_type == METRIC_INNER_PRODUCT) {
            if (verbose) {
                printf("IndexIVFPQAdaptive::precompute_table: precomputed "
                       "tables not needed for inner product quantizers\n");
            }
            precomputed_table.resize(0);
            return;
        }
        const MultiIndexQuantizerAdatpive* miq =
                dynamic_cast<const MultiIndexQuantizerAdatpive*>(quantizer);
        if (miq && pq.M % miq->pq.M == 0)
            ...
            if (table_size > precomputed_table_max_bytes) {
                if (verbose) {
                    printf("IndexIVFPQAdaptive::precompute_table: not precomputing table, "
    ...
    if (use_precomputed_table == 1) {
        ...
        const ProductQuantizerAdaptive& cpq = miq->pq;
...
void IndexIVFPQAdaptive::precompute_table() {
 ...
 * IndexIVFPQAdaptive. The code contains a lot of branches, depending on:
...
struct QueryTables {
    ...
    const IndexIVFPQAdaptive& ivfpq;
    const IVFSearchParameters* params;

    // copied from IndexIVFPQAdaptive for easier access
    int d;
    const ProductQuantizerAdaptive& pq;
    ...
    float precompute_list_tables_L2() {
        ...
        if (use_precomputed_table == 0 || use_precomputed_table == -1) {
            ...
            const MultiIndexQuantizerAdatpive* miq =
                    dynamic_cast<const MultiIndexQuantizerAdatpive*>(ivfpq.quantizer);
            FAISS_THROW_IF_NOT(miq);
            const ProductQuantizerAdaptive& cpq = miq->pq;
            int Mf = pq.M / cpq.M;
...
struct IVFPQScannerT : QueryTables {
    ...
    IVFPQScannerT(const IndexIVFPQAdaptive& ivfpq, const IVFSearchParameters* params)
...
struct IVFPQScanner : IVFPQScannerT<Index::idx_t, METRIC_TYPE, PQDecoder>,
    ...
    IVFPQScanner(const IndexIVFPQAdaptive& ivfpq, bool store_pairs, int precompute_mode)
...
InvertedListScanner* get_InvertedListScanner1(
        const IndexIVFPQAdaptive& index,
        bool store_pairs) {
...
InvertedListScanner* IndexIVFPQAdaptive::get_InvertedListScanner(
...
IndexIVFPQAdaptiveStats indexIVFPQ_stats;

void IndexIVFPQAdaptiveStats::reset() {
...
size_t IndexIVFPQAdaptive::find_duplicates(idx_t* dup_ids, size_t* lims) const {
    size_t ngroup = 0;
    lims[0] = 0;
