...
void fvec2bitvec_p(const float* x, uint8_t* b, size_t d, const int* partitions) {
    for (int i = 0; i < d; i += 8) {
        uint8_t w = 0;
        uint8_t mask = 1;
        int nj = i + 8 <= d ? 8 : d - i;
        for (int j = 0; j < nj; j++) {
            size_t cur_p = i + j;
            size_t s = partitions[cur_p];
            size_t e = partitions[cur_p + 1];
            if (e - s == 1) {
                if (x[s] >= 0)
                    w |= mask;
            } else {
...
// #pragma omp parallel for if (n > 100000)
    for (int64_t i = 0; i < n; i++)
       fvec2bitvec_p(x + i * d, b + i * ncodes, d, partitions);
}
