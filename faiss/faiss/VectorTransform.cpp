...
void FlyMatrix::init(const float* x) {
    A.resize(d_out * d_in);
...
void FlyMatrix::train(Index::idx_t n, const float* x) {
...
OPQMatrixAdaptive::OPQMatrixAdaptive(int d, int M, int d2)
        : LinearTransform(d, d2 == -1 ? d : d2, false),
          M(M),
          niter(50),
          niter_pq(4),
...
void OPQMatrixAdaptive::set_prob(idx_t n, const double* p){
    for(int i = 0 ; i < n; i++){
        prob.push_back(p[i]);
    }
...
void OPQMatrixAdaptive::train(Index::idx_t n, const float* x) {
    const float* x_in = x;
    ...
    if (verbose) {
        printf("OPQMatrixAdaptive::train: training an OPQ rotation matrix "
               "for M=%d from %" PRId64 " vectors in %dD -> %dD\n",
               M,
               n,
               d_in,
    ...
    if (A.size() == 0) {
        ...
        rotation = A.data();
        if (verbose)
            printf("  OPQMatrixAdaptive::train: making random %zd*%zd rotation\n",
                   d,
                   d);
        float_randn(rotation, d * d, 1234);
        matrix_qr(d, d, rotation);
    ...
    ProductQuantizerAdaptive pq_default(d2, M, 8);
    ProductQuantizerAdaptive& pq_regular = pq ? *pq : pq_default;
    std::vector<uint8_t> codes(pq_regular.code_size * n);
    ...
    for (int iter = 0; iter < niter; iter++) {
        ...
        if (verbose) {
        printf("xtrain size %ld d2 %ld xproj size %ld prob size %ld \n",xtrain.size(),d2,xproj.size(), prob.size());
        }
        pq_regular.cp.max_points_per_centroid = 1000;
        pq_regular.cp.niter = iter == 0 ? niter_pq_0 : niter_pq;
        pq_regular.verbose = verbose;
        pq_regular.train(n, xproj.data(),prob.data());
        ...
        pq_regular.train_type = ProductQuantizerAdaptive::Train_hot_start;
    }
