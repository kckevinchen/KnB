...
#include <faiss/IndexNSGAdaptive.h>
...
using namespace nsgAdaptive;

/**************************************************************
 * IndexNSG implementation
 **************************************************************/

IndexNSGAdaptive::IndexNSGAdaptive(int d, int R, MetricType metric)
        : Index(d, metric),
          nsg(R),
          own_fields(false),
          storage(nullptr),
...
IndexNSGAdaptive::IndexNSGAdaptive(Index* storage, int R)
...
IndexNSGAdaptive::~IndexNSGAdaptive() {
    if (own_fields) {
        delete storage;
    }
}

void IndexNSGAdaptive::train(idx_t n, const float* x) {
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "Please use IndexNSGFlat (or variants) instead of IndexNSGAdaptive directly");
    // nsg structure does not require training
    storage->train(n, x);
    is_trained = true;
}

void IndexNSGAdaptive::search(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
...
#pragma omp parallel
        {
            ...
            DistanceComputer* dis = storage_distance_computerAdaptive(storage);
...
void IndexNSGAdaptive::build(idx_t n, const float* x, idx_t* knn_graph, int GK) {
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "Please use IndexNSGFlat (or variants) instead of IndexNSGAdaptive directly");
    FAISS_THROW_IF_NOT_MSG(
            !is_built && ntotal == 0, "The IndexNSGAdaptive is already built");
    ...
    const nsgAdaptive::Graph<idx_t> knng(knn_graph, n, GK);
    nsg.build(storage, n, knng, verbose);
    is_built = true;
}

void IndexNSGAdaptive::add(idx_t n, const float* x) {
    FAISS_THROW_IF_NOT_MSG(
            storage,
...
#pragma omp parallel for
    ...
    const nsgAdaptive::Graph<idx_t> knn_graph(knng.data(), n, GK);
    nsg.build(storage, n, knn_graph, verbose);
    is_built = true;
}

void IndexNSGAdaptive::reset() {
    nsg.reset();
    storage->reset();
    ntotal = 0;
    is_built = false;
...
void IndexNSGAdaptive::reconstruct(idx_t key, float* recons) const {
    storage->reconstruct(key, recons);
}

void IndexNSGAdaptive::check_knn_graph(const idx_t* knn_graph, idx_t n, int K) const {
    idx_t total_count = 0;
...
void IndexNSGAdaptive::add_with_prob(
        idx_t n,
        const float* x,
        const double* prob) {
    FAISS_THROW_IF_NOT_MSG(
...
#pragma omp parallel for
    ...
    printf("IndexNSG::add %zd vectors\n", size_t(n));
    nsg.build_with_prob(storage,prob, n, knn_graph, verbose);
    is_built = true;
}
...
IndexNSGFlatAdaptive::IndexNSGFlatAdaptive() {
    is_trained = true;
}

IndexNSGFlatAdaptive::IndexNSGFlatAdaptive(int d, int R, MetricType metric)
        : IndexNSGAdaptive(new IndexFlat(d, metric), R) {
    own_fields = true;
    is_trained = true;
}
