...
struct HNSWAdaptive {
    ...
    /// number of neighbors stored per nodes
    ...
    int random_level_adptive(double f = 0.);

    void preset_level(size_t n, const double* prob);
    ...
    int nb_neighbors_adaptive(idx_t no, int layer_no)const;

    /// only mandatory parameter: nb of neighbors
    explicit HNSWAdaptive(int M = 32);
...
struct HNSWAdaptiveStats {
    ...
    void combine(const HNSWAdaptiveStats& other) {
        n1 += other.n1;
        n2 += other.n2;
        n3 += other.n3;
        ndis += other.ndis;
...
FAISS_API extern HNSWAdaptiveStats hnsw_adaptive_stats;
