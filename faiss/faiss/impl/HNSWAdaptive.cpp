...
int HNSWAdaptive::nb_neighbors(int layer_no) const {
    return cum_nneighbor_per_level[layer_no + 1] -
            cum_nneighbor_per_level[layer_no];
}

void HNSWAdaptive::set_nb_neighbors(int level_no, int n) {
...
int HNSWAdaptive::cum_nb_neighbors(int layer_no) const {
    return cum_nneighbor_per_level[layer_no];
}

int HNSWAdaptive::nb_neighbors_adaptive(idx_t no, int layer_no) const {
    return nb_neighbors(layer_no) + neighbor_offsets[no];
}

void HNSWAdaptive::neighbor_range(idx_t no, int layer_no, size_t* begin, size_t* end)
        const {
    size_t o = offsets[no];
...
HNSWAdaptive::HNSWAdaptive(int M) : rng(12345) {
...
int HNSWAdaptive::random_level_adptive(double f) {
    if(f == 0){
        f = rng.rand_float();
    }
    // could be a bit faster with bissection
...
void HNSWAdaptive::preset_level(size_t n, const double* prob) {
    size_t n0 = offsets.size() - 1;
    FAISS_ASSERT(n0 == levels.size());
    for (int i = 0; i < n; i++) {
        int pt_level = random_level_adptive(prob[i]);
        levels.push_back(pt_level + 1);
    }
}
void HNSWAdaptive::set_default_probas(int M, float levelMult) {
    int nn = 0;
...
void HNSWAdaptive::clear_neighbor_tables(int level) {
...
void HNSWAdaptive::print_neighbor_stats(int level) const {
...
void HNSWAdaptive::fill_with_random_links(size_t n) {
...
int HNSWAdaptive::prepare_level_tab(size_t n, bool preset_levels) {
    size_t n0 = offsets.size() - 1;
 ...
 */
void HNSWAdaptive::shrink_neighbor_list(
...
using NodeDistCloser = HNSWAdaptive::NodeDistCloser;
...
void shrink_neighbor_list(
    ...
    HNSWAdaptive::shrink_neighbor_list(qdis, resultSet, returnlist, max_size);
...
void HNSWAdaptive::add_links_starting_from(
...
void HNSWAdaptive::add_with_locks(
        DistanceComputer& ptdis,
        int pt_level,
        int pt_id,
...
int HNSWAdaptive::search_from_candidates(
        DistanceComputer& qdis,
        int k,
        idx_t* I,
        float* D,
...
std::priority_queue<HNSWAdaptive::Node> HNSWAdaptive::search_from_candidate_unbounded(
        const Node& node,
        DistanceComputer& qdis,
        int ef,
        VisitedTable* vt,
        HNSWAdaptiveStats& stats) const {
    int ndis = 0;
...
HNSWAdaptiveStats HNSWAdaptive::search(
        DistanceComputer& qdis,
        int k,
        idx_t* I,
        float* D,
...
void HNSWAdaptive::MinimaxHeap::push(storage_idx_t i, float v) {
    if (k == n) {
        if (v >= dis[0])
            return;
...
float HNSWAdaptive::MinimaxHeap::max() const {
    return dis[0];
}

int HNSWAdaptive::MinimaxHeap::size() const {
    return nvalid;
}

void HNSWAdaptive::MinimaxHeap::clear() {
    nvalid = k = 0;
}

int HNSWAdaptive::MinimaxHeap::pop_min(float* vmin_out) {
    assert(k > 0);
...
int HNSWAdaptive::MinimaxHeap::count_below(float thresh) {
    int n_below = 0;
    for (int i = 0; i < k; i++) {
        if (dis[i] < thresh) {
            n_below++;
