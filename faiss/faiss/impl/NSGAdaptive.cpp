...
DistanceComputer* storage_distance_computerAdaptive(const Index* storage) {
...
NSGAdaptive::NSGAdaptive(int R) : R(R), rng(0x0903) {
    L = R + 32;
    C = R + 100;
    search_L = 16;
    ntotal = 0;
...
void NSGAdaptive::build(
        Index* storage,
        idx_t n,
        const nsgAdaptive::Graph<idx_t>& knn_graph,
        bool verbose) {
    if (verbose) {
    ...
    {
        nsgAdaptive::Graph<Node> tmp_graph(n, R);

        link(storage, knn_graph, tmp_graph, verbose);

        final_graph = std::make_shared<nsgAdaptive::Graph<int>>(n, R);
...
void NSGAdaptive::reset() {
    final_graph.reset();
    ntotal = 0;
    is_built = false;
}

void NSGAdaptive::init_graph(
        Index* storage,
        const nsgAdaptive::Graph<idx_t>& knn_graph) {
    int d = storage->d;
    ...
    std::unique_ptr<DistanceComputer> dis(storage_distance_computerAdaptive(storage));
...
void NSGAdaptive::search_on_graph(
        const nsgAdaptive::Graph<index_t>& graph,
        DistanceComputer& dis,
        VisitedTable& vt,
        int ep,
        int pool_size,
...
void NSGAdaptive::link(
        Index* storage,
        const nsgAdaptive::Graph<idx_t>& knn_graph,
        nsgAdaptive::Graph<Node>& graph,
        bool /* verbose */) {
#pragma omp parallel
    {
        ...
        std::unique_ptr<DistanceComputer> dis(
                storage_distance_computerAdaptive(storage));
...
void NSGAdaptive::add_reverse_links(
        int q,
...
int NSGAdaptive::tree_grow(Index* storage, std::vector<int>& degrees) {
    int root = enterpoint;
    VisitedTable vt(ntotal);
...
int NSGAdaptive::dfs(VisitedTable& vt, int root, int cnt) const {
    int node = root;
    std::stack<int> stack;
    stack.push(root);
...
int NSGAdaptive::attach_unlinked(
        Index* storage,
        VisitedTable& vt,
        VisitedTable& vt2,
...
void NSGAdaptive::check_graph() const {
#pragma omp parallel for
    for (int i = 0; i < ntotal; i++) {
        for (int j = 0; j < R; j++) {
...
void NSGAdaptive::build_with_prob(
        Index* storage,
        const double* prob,
        idx_t n,
        const nsgAdaptive::Graph<idx_t>& knn_graph,
    ...
    ntotal = n;
    init_graph_with_prob(storage, prob, knn_graph);
    printf("enterpoint %d \n", enterpoint);
    build(storage, n, knn_graph, true);
}

void NSGAdaptive::init_graph_with_prob(
        Index* storage,
        const double* prob,
        const nsgAdaptive::Graph<idx_t>& knn_graph) {
    int d = storage->d;
...
// NSGAdaptive::NSGAdaptive(int R) : NSG(R) {}
...
//     dis(storage_distance_computer(storage));
