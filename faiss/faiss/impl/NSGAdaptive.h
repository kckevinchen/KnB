...
struct NSGAdaptive {
    ...
    std::shared_ptr<nsgAdaptive::Graph<int>>
            final_graph; ///< NSG graph structure

    bool is_built; ///< NSG is built or not

    RandomGenerator rng; ///< random generator

    explicit NSGAdaptive(int R = 32);
    ...
    // Compute the center point
    void init_graph(Index* storage, const nsgAdaptive::Graph<idx_t>& knn_graph);
