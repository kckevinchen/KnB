...
#include <faiss/IndexPQAdaptive.h>
...
IndexPQAdaptive::IndexPQAdaptive(int d, size_t M, size_t nbits, MetricType metric)
...
IndexPQAdaptive::IndexPQAdaptive() {
...
void IndexPQAdaptive::preset_level(int n, const int *assign_level, int max_level, const int *budget_per_level){
    pq.preset_level(n, assign_level,max_level, budget_per_level);
}

void IndexPQAdaptive::train(idx_t n, const float* x) {
    if (!do_polysemous_training) { // standard training
        if(prob.size()==n){
            printf("multi-level training \n");

            pq.train(n, x,prob.data());
        }
        else{
            pq.train(n, x);
        }
...
void IndexPQAdaptive::set_prob(idx_t n, const double* p){
    for(int i = 0 ; i < n; i++){
        prob.push_back(p[i]);
    }

}
void IndexPQAdaptive::train_with_prob(idx_t n, const float* x,const double* p,const double c) {
    if (!do_polysemous_training) { // standard training
        pq.train(n, x,p,c);
    } else {
            printf("unsupport");
...
struct PQDistanceComputer : DistanceComputer {
    ...
    explicit PQDistanceComputer(const IndexPQAdaptive& storage) : pq(storage.pq) {
...
DistanceComputer* IndexPQAdaptive::get_distance_computer() const {
    if (pq.nbits == 8) {
...
void IndexPQAdaptive::search(
    ...
    if (search_type == ST_PQ) { // Simple PQ search
        ...
        indexPQAdaptive_stats.nq += n;
        indexPQAdaptive_stats.ncode += n * ntotal;
...
void IndexPQAdaptiveStats::reset() {
    nq = ncode = n_hamming_pass = 0;
}

IndexPQAdaptiveStats indexPQAdaptive_stats;

template <class HammingComputer>
static size_t polysemous_inner_loop(
        const IndexPQAdaptive& index,
...
void IndexPQAdaptive::search_core_polysemous(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
...
#pragma omp parallel for reduction(+ : n_pass)
    ...
    indexPQAdaptive_stats.nq += n;
    indexPQAdaptive_stats.ncode += n * ntotal;
    indexPQAdaptive_stats.n_hamming_pass += n_pass;
}

/* The standalone codec interface (just remaps to the PQ functions) */

void IndexPQAdaptive::sa_encode(idx_t n, const float* x, uint8_t* bytes) const {
    pq.compute_codes(x, bytes, n);
}

void IndexPQAdaptive::sa_decode(idx_t n, const uint8_t* bytes, float* x) const {
    pq.decode(bytes, x, n);
}
...
void IndexPQAdaptive::hamming_distance_table(idx_t n, const float* x, int32_t* dis)
        const {
    uint8_t* q_codes = new uint8_t[n * pq.code_size];
    ScopeDeleter<uint8_t> del(q_codes);
...
void IndexPQAdaptive::hamming_distance_histogram(
        idx_t n,
        const float* x,
        idx_t nb,
        const float* xb,
...
MultiIndexQuantizerAdatpive::MultiIndexQuantizerAdatpive(int d, size_t M, size_t nbits)
        : Index(d, METRIC_L2), pq(d, M, nbits) {
    is_trained = false;
    pq.verbose = verbose;
}

void MultiIndexQuantizerAdatpive::train(idx_t n, const float* x) {
    pq.verbose = verbose;
    pq.train(n, x);
    is_trained = true;
...
void MultiIndexQuantizerAdatpive::search(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
    ...
    if (n > bs) {
        for (idx_t i0 = 0; i0 < n; i0 += bs) {
            ...
            if (verbose) {
                printf("MultiIndexQuantizerAdatpive::search: %" PRId64 ":%" PRId64
                       " / %" PRId64 "\n",
                       i0,
                       i1,
                       n);
...
void MultiIndexQuantizerAdatpive::reconstruct(idx_t key, float* recons) const {
    int64_t jj = key;
    for (int m = 0; m < pq.M; m++) {
...
void MultiIndexQuantizerAdatpive::add(idx_t /*n*/, const float* /*x*/) {
    FAISS_THROW_MSG(
            "This index has virtual elements, "
            "it does not support add");
}

void MultiIndexQuantizerAdatpive::reset() {
    FAISS_THROW_MSG(
            "This index has virtual elements, "
...
MultiIndexQuantizerAdatpive2::MultiIndexQuantizerAdatpive2(
        int d,
        size_t M,
        size_t nbits,
        Index** indexes)
        : MultiIndexQuantizerAdatpive(d, M, nbits) {
...
MultiIndexQuantizerAdatpive2::MultiIndexQuantizerAdatpive2(
        ...
        : MultiIndexQuantizerAdatpive(d, 2, nbits) {
    FAISS_THROW_IF_NOT_MSG(
...
void MultiIndexQuantizerAdatpive2::train(idx_t n, const float* x) {
    MultiIndexQuantizerAdatpive::train(n, x);
...
void MultiIndexQuantizerAdatpive2::search(
        idx_t n,
        const float* x,
        idx_t K,
        float* distances,
